# 🏠 ООП — Свойства: property() и @property
...
Простая шпаргалка — что и когда вызывается:
...
**Решение 1:** дублировать проверку в `__init__` — плохо, код повторяется.

**Решение 2:** обращаться в `__init__` к свойству `self.name`, а не к атрибуту `self._name` — тогда сеттер вызовется автоматически:
...
Это место сбивает с толку многих — имена похожи, но это совершенно разные вещи.
...
> 💡 `obj._name is obj.name` вернёт `True` — но только потому что геттер возвращает тот же самый `obj._name`. Это не значит что они одно и то же!

---

## 🔍 Порядок создания: почему self.name в __init__ вызывает сеттер?

Это ещё один момент который многих удивляет. Ответ в том, **когда именно создаётся свойство**.
...
class Cat:
    def __init__(self, name):
        self.name = name       # свойство уже существует → вызывается сеттер
...
# Убедимся что свойство создаётся при определении класса:
print(Cat.__init__)   # → <function Cat.__init__ at 0x...>
print(Cat.name)       # → <property object at 0x...>  ← уже есть!

# Только после этого создаём экземпляр:
cat = Cat('Кемаль')   # __init__ вызывается здесь, свойство уже готово
```
