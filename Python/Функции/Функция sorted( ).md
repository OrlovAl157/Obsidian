...
```
# Сортировка через лямбду по двум условиям

## Основной синтаксис

```python
data.sort(key=lambda x: (условие1, условие2))
```

## Примеры

### Пример 1: Сортировка по возрасту, затем по имени
...
# Результат: сначала по возрасту, потом по имени
```

### Пример 2: По убыванию первого, по возрастанию второго
...
# Минус для обратной сортировки первого условия
```

### Пример 3: Булево условие + число

```python
tasks = [
    {'task': 'Задача1', 'done': False, 'priority': 2},
    {'task': 'Задача2', 'done': True, 'priority': 1},
    {'task': 'Задача3', 'done': False, 'priority': 1}
]

tasks.sort(key=lambda x: (x['done'], x['priority']))
# False идёт перед True, затем по приоритету
```

## Ключевые моменты

- **Кортеж** в лямбде: `(условие1, условие2)`
- **Порядок важен**: сначала сортирует по первому, при равенстве - по второму
- **Реверс одного условия**: используй минус `-x` для чисел или `not x` для булевых
- **sorted()** вместо sort() создаёт новый списоку
### Сортировка словарей
