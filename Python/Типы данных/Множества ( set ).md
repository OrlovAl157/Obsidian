# Памятка: Множества в Python

## 💡 Полезные советы

- **Множества изменяемы**: можно добавлять и удалять элементы (но не изменять существующие)
- **Только уникальные элементы**: дубликаты автоматически удаляются
- **Неупорядоченные**: порядок элементов не гарантируется
- **Только неизменяемые элементы**: можно хранить числа, строки, кортежи (но не списки или словари)
- **Быстрый поиск**: проверка `x in set` работает очень быстро (O(1))
- **Математические операции**: объединение, пересечение, разность, симметричная разность
- **frozenset**: неизменяемая версия множества, можно использовать как ключ словаря
- **Используйте set comprehension**: `{x for x in ...}` для создания множеств

## 🚀 Быстрая справка

| Задача             | Метод/Оператор                    | Пример               |
| ------------------ | --------------------------------- | -------------------- |
| Создать множество  | `{}` или `set()`                  | `{1, 2, 3}`          |
| Добавить элемент   | `.add()`                          | `set.add(5)`         |
| Добавить несколько | `.update()`                       | `set.update([4, 5])` |
| Удалить элемент    | `.remove()`, `.discard()`         | `set.remove(5)`      |
| Удалить случайный  | `.pop()`                          | `set.pop()`          |
...
| Подмножество       | `<=` или `.issubset()`            | `set1 <= set2`       |
| Надмножество       | `>=` или `.issuperset()`          | `set1 >= set2`       |
| Копировать         | `.copy()`                         | `set.copy()`         |
