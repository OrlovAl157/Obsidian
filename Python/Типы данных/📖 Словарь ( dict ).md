# Памятка: Словари в Python

## 💡 Полезные советы

- **Словари изменяемы**: можно добавлять, изменять и удалять пары ключ-значение
- **Ключи должны быть неизменяемыми**: строки, числа, кортежи (но не списки)
- **Ключи уникальны**: дублирующиеся ключи перезаписываются
- **Быстрый доступ**: поиск по ключу работает очень быстро (O(1))
- **Порядок сохраняется**: с Python 3.7+ словари сохраняют порядок добавления
- **Используйте .get()**: безопасный доступ к ключам без ошибок
- **Dict comprehension**: `{k: v for k, v in ...}` для создания словарей
- **Методы .keys(), .values(), .items()**: для итерации по словарю

## 🚀 Быстрая справка [[Методы dict {}]]

| Задача                | Метод/Оператор       | Пример                    |
| --------------------- | -------------------- | ------------------------- |
| Создать словарь       | `{}` или `dict()`    | `{"a": 1, "b": 2}`        |
| Добавить/изменить     | `[key] = value`      | `d["c"] = 3`              |
| Получить значение     | `[key]` или `.get()` | `d["a"]` или `d.get("a")` |
| Удалить элемент       | `del` или `.pop()`   | `del d["a"]`              |
| Проверить ключ        | `in`                 | `"a" in d`                |
| Все ключи             | `.keys()`            | `d.keys()`                |
| Все значения          | `.values()`          | `d.values()`              |
| Пары ключ-значение    | `.items()`           | `d.items()`               |
| Объединить словари    | `.update()` или `    | `d1.update(d2)`           |
| Очистить              | `.clear()`           | `d.clear()`               |
| Копировать            | `.copy()`            | `d.copy()`                |
| Длина                 | `len()`              | `len(d)`                  |
| Значение по умолчанию | `.setdefault()`      | `d.setdefault("a", 0)`    |
| Удалить и вернуть     | `.pop(key)`          | `d.pop("a")`              |
