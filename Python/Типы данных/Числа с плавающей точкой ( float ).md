# Памятка: Числа с плавающей точкой (float) в Python

## 💡 Полезные советы

- **Точность**: float имеет ограниченную точность (~15-17 значащих цифр)
- **Проблема 0.1 + 0.2**: из-за двоичного представления не равно 0.3!
- **Для денег**: используйте Decimal, не float
- **Сравнение**: используйте `math.isclose()`, не `==`
- **Деление**: `/` всегда возвращает float, даже для целых чисел
- **Специальные значения**: `inf` (бесконечность) и `nan` (Not a Number)
- **Научная нотация**: `1.5e2` = 150.0, `1.5e-2` = 0.015
- **Преобразование**: `float()` для конвертации из строк и чисел

## 🚀 Быстрая справка

|Задача|Функция/Оператор|Пример|
|---|---|---|
|Создать float|`float()`|`float("3.14")` → `3.14`|
|Из int|`float(int)`|`float(42)` → `42.0`|
|Сложение|`+`|`3.5 + 2.1` → `5.6`|
|Вычитание|`-`|`5.5 - 2.3` → `3.2`|
|Умножение|`*`|`2.5 * 4` → `10.0`|
|Деление|`/`|`7 / 2` → `3.5`|
|Степень|`**`|`2.5 ** 2` → `6.25`|
|Округление|`round()`|`round(3.7)` → `4`|
|Модуль|`abs()`|`abs(-3.5)` → `3.5`|
|Проверка на int|`.is_integer()`|`(3.0).is_integer()` → `True`|
|Проверка на NaN|`math.isnan()`|`math.isnan(float('nan'))` → `True`|
|Проверка на inf|`math.isinf()`|`math.isinf(float('inf'))` → `True`|
|Сравнение с погрешностью|`math.isclose()`|`math.isclose(0.1+0.2, 0.3)` → `True`|
...
### Проценты и процентное соотношение
...
round_to_decimals(3.14159, 2)   # → 3.14
round_to_decimals(123.456, 1)   # → 123.5
