---
tags: [python, logging, handler, formatter, logger, архитектура]
difficulty: intermediate
---

# 🔧 Logger, Handler и Formatter — устройство

> Архитектура модуля logging изнутри: как LogRecord проходит путь от вызова `logger.info()` до записи в файл или консоль, как взаимодействуют Logger, Handler, Formatter и Filter, и что происходит на каждом шаге.

## Содержание

- [[#Справка: основные объекты|Справка]]
- [[#Архитектура: основная схема|Общая схема]]
- [[#Жизненный цикл `logger.info()`|Жизненный цикл сообщения]]
- [[#Поиск обработчиков: механизм propagate|Механизм propagate]]
- [[#Структура LogRecord|Структура LogRecord]]
- [[#Как Formatter использует LogRecord|Formatter и LogRecord]]
- [[#Несколько Handler на одном Logger|Несколько Handler]]
- [[#Поле extra: добавление пользовательских данных|Поле extra]]
- [[#Уровни логирования и фильтрация|Уровни и фильтрация]]
- [[#Практический пример: полная схема|Практический пример]]
- [[#Главные правила (Запомнить)|Главные правила]]
...
Filter — промежуточный этап между Logger/Handler и обработкой. Подробнее → [[04 — 🔍 Фильтрация событий]]
...
#python/logging #архитектура #LogRecord #handler #formatter
