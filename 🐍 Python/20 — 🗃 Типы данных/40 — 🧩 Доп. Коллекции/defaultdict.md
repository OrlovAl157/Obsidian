...
- `list` — фабрика: когда ключа нет → вызывает `list()` → возвращает `[]`
- Состояние: `d = {}`


### `d['a'].append(1)` — новый ключ

**Шаг 1:** Ищем ключ `'a'` → **не найден**

**Шаг 2:** Вызываем фабрику `list()` → получаем `[]`

**Шаг 3:** Записываем `d['a'] = []`

**Шаг 4:** Вызываем `.append(1)` → `d['a'] = [1]`

- Состояние: `d = {'a': [1]}`


### `d['a'].append(2)` — существующий ключ

**Шаг 1:** Ищем ключ `'a'` → **найден** `[1]`

**Шаг 2:** Вызываем `.append(2)` → `d['a'] = [1, 2]`

- Состояние: `d = {'a': [1, 2]}`


### `d['b'].append(3)` — новый ключ

**Шаг 1:** Ищем ключ `'b'` → **не найден**

**Шаг 2:** Вызываем фабрику `list()` → получаем `[]`

**Шаг 3:** Записываем `d['b'] = []`

**Шаг 4:** Вызываем `.append(3)` → `d['b'] = [3]`

- Состояние: `d = {'a': [1, 2], 'b': [3]}`
