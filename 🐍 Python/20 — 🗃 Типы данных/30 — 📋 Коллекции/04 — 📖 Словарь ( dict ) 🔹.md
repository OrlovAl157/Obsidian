...
> `dict` — изменяемая коллекция пар «ключ: значение» на основе хэш-таблиц. Упорядоченная (Python 3.7+). Структура памяти: `(hash, key, value)` + указатели порядка. Поиск O(1).

## Содержание

- [[#Справка (Методы)|Справка]]
...
- [[#🎯 Когда использовать словари|Когда использовать]]
