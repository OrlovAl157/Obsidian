...
`list` — это упорядоченная, изменяемая коллекция элементов произвольных типов.

### 💡 Ключевые особенности

- **Изменяемость (Mutable):** Можно добавлять, удалять и менять элементы «на месте».  
    
- **Упорядоченность:** Элементы хранятся в порядке добавления.  
    
- **Динамичность:** Автоматически расширяется при добавлении данных.  
    
- **Дубликаты:** Может содержать одинаковые значения.  
...
| ---------------------------- | -------------------- | ----------------- | ----------------------------- |
| **Добавить в конец**         | `.append(x)`         | O(1)              | `l.append(5)`                 |
| **Расширить другим списком** | `.extend(iterable)`  | O(k)              | `l.extend([1, 2])`            |
| **Вставить по индексу**      | `.insert(i, x)`      | O(n)              | `l.insert(0, "start")`        |
| **Удалить по значению**      | `.remove(x)`         | O(n)              | `l.remove("apple")`           |
| **Удалить по индексу**       | `.pop(i)`            | O(n)              | `last = l.pop()` (i=-1: O(1)) |
| **Очистить всё**             | `.clear()`           | O(n)              | `l.clear()`                   |
| **Найти индекс**             | `.index(x)`          | O(n)              | `idx = l.index(5)`            |
| **Количество вхождений**     | `.count(x)`          | O(n)              | `cnt = l.count(1)`            |
| **Отсортировать**            | `.sort()`            | O(n log n)        | `l.sort()`                    |
| **Перевернуть**              | `.reverse()`         | O(n)              | `l.reverse()`                 |
| **Длина списка**             | `len()`              | O(1)              | `len(l)`                      |
| **Копировать**               | `.copy()` или `[:]`  | O(n)              | `l.copy()`                    |
| **Срез**                     | `[start:end:step]`   | O(k)              | `l[1:5:2]`                    |
