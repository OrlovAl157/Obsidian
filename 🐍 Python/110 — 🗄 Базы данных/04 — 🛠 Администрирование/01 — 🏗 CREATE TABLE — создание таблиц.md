...
**Наглядно — что произойдёт с дочерней таблицей при `DELETE FROM Authors WHERE id = 1`, в зависимости от выбранной опции:**
...
| Опция | Что стало с `Books` после удаления автора `id=1` |
|---|---|
| `ON DELETE CASCADE` | строка `id=1` в `Books` тоже удалена — книга исчезла вместе с автором |
| `ON DELETE SET NULL` | строка осталась, но `author_id` стал `NULL` — книга без автора |
| `ON DELETE RESTRICT` (по умолчанию) | удаление `Authors` вообще не произошло — СУБД выдала ошибку, пока есть зависимая книга |

Логика простая: `CASCADE` — "удалить вместе", `SET NULL` — "оставить, но разорвать связь", `RESTRICT` — "не позволить удалить, пока есть зависимости". `ON UPDATE` работает по той же схеме, только триггером служит не `DELETE`, а `UPDATE` значения `id` в родительской таблице.
