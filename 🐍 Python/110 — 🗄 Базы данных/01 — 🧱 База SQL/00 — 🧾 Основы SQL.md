---
tags: [sql, база_данных, основы_sql]
difficulty: beginner
---
...
- [[#🔴 WHERE — фильтрация данных|WHERE]]
- [[#🟡 AND / OR / IN / NOT — логическая фильтрация|AND / OR / IN / NOT]]
- [[#✂️ LIKE — шаблонный поиск|LIKE]]
- [[#🔤 Регистрозависимость|Регистрозависимость]]
- [[#📐 Вычисляемые поля|Вычисляемые поля]]
- [[#🔗 Объединение значений полей|CONCAT и CONCAT_WS]]
...
| Команда | Назначение | Где в запросе |
|---|---|---|
| `SELECT col1, col2` | выбрать столбцы | обязательно |
| `FROM table` | указать таблицу | обязательно |
| `WHERE условие` | фильтровать строки | после FROM |
| `AND / OR` | объединить условия | внутри WHERE |
| `IN (a, b, c)` | проверить вхождение в список | внутри WHERE |
| `BETWEEN a AND b` | диапазон включая границы | внутри WHERE |
| `IS NULL / IS NOT NULL` | проверить NULL | внутри WHERE |
| `LIKE 'шаблон'` | поиск по шаблону | внутри WHERE |
| `ORDER BY col ASC/DESC` | сортировать результат | после WHERE |
| `LIMIT n` | ограничить количество строк | в конце |
| `LIMIT offset, n` | пропустить offset, взять n | в конце |
| `DISTINCT` | убрать дубликаты | после SELECT |
| `CONCAT(...)` | объединить строки | в SELECT |
| `CONCAT_WS(sep, ...)` | объединить через разделитель | в SELECT |
...
## 🟡 AND / OR / IN / NOT — логическая фильтрация
