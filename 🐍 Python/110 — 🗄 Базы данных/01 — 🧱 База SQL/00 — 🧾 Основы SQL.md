...
```sql
-- ============================================
-- ПРИМЕР 1. Обычный запрос с LIMIT
-- ============================================

-- Выбираем два столбца: название фильма и режиссёра
SELECT title, director
-- Из таблицы Films
FROM Films
-- Ограничиваем вывод первыми 5 строками
LIMIT 5;
...
--
-- ⚠️ Порядок строк без ORDER BY не гарантирован!


-- ============================================
-- ПРИМЕР 2. LIMIT с сортировкой (стабильный результат)
-- ============================================

-- Выбираем те же два столбца
SELECT title, director
-- Из таблицы Films
FROM Films
-- Сортируем по названию, чтобы порядок был предсказуемым
ORDER BY title
-- И только потом берём первые 5 строк
LIMIT 5;


-- ============================================
-- ПРИМЕР 3. LIMIT offset, n — пропуск строк
-- ============================================
...
-- Из таблицы Films
FROM Films
-- Сортируем по названию, чтобы порядок был стабильным
ORDER BY title
-- Пропускаем первые 5 строк и выводим следующие 5
LIMIT 5, 5;

-- Что происходит:
--   5 до запятой   — offset: пропустить первые 5 строк
--   5 после запятой — n: вывести следующие 5 строк
--
-- Это удобно для пагинации (постраничного вывода):
--   страница 1 → LIMIT 0, 5
--   страница 2 → LIMIT 5, 5
--   страница 3 → LIMIT 10, 5
--   и так далее


-- ============================================
-- 🧠 МИНИ-ШПАРГАЛКА: порядок ключевых слов
-- ============================================

SELECT   столбцы
FROM     таблица
WHERE    условие          -- (необязательно)
ORDER BY столбец          -- (необязательно, но нужно для стабильного порядка)
LIMIT    n                -- (необязательно)
OFFSET   m;               -- (необязательно)
```
