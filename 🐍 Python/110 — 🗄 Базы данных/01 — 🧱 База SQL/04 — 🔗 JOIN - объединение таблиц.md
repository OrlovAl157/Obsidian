...
- [[#🔑 USING — сокращённый синтаксис|USING]]
...
## 🔑 USING — сокращённый синтаксис

**Назначение:** упрощённая запись `ON` когда столбцы в обеих таблицах называются **одинаково**.

| Конструкция | Что делает |
|---|---|
| `JOIN table2 USING (col)` | Как `ON t1.col = t2.col` но короче |
| `JOIN table2 USING (col1, col2)` | Несколько общих столбцов |

**Примеры:**

```sql
-- Обычный синтаксис через ON
SELECT u.name, o.amount
FROM Users u
JOIN Orders o ON u.id = o.id;

-- То же самое через USING (столбец называется одинаково!)
SELECT u.name, o.amount
FROM Users u
...
SELECT *
FROM Orders o
JOIN OrderItems oi USING (order_id, shop_id);
```
...
| | `ON` | `USING` |
|---|---|---|
| Когда использовать | Всегда | Только если столбцы называются одинаково |
| Столбец в результате | Дублируется (из обеих таблиц) | Появляется один раз |
| Гибкость | Высокая | Только равенство |
| Читаемость | Явная | Компактная |

**Важное:**
- `USING` работает только если столбец называется **одинаково** в обеих таблицах
- В результате столбец из `USING` появляется **один раз** (не дублируется как при `ON`)
- Не все СУБД поддерживают `USING` — MySQL и PostgreSQL поддерживают, SQLite тоже
...
- [SQL JOIN — визуальное объяснение](https://antonz.ru/sql-join/)