...
| Что нужно | Код |
|---|---|
| Создать интервал | `timedelta(days=5, hours=3)` |
| Общая длительность в секундах | `td.total_seconds()` |
| Прибавить к дате | `date_or_dt + timedelta(...)` |
| Разница между датами | `d1 - d2` → `timedelta` |
| Сложить интервалы | `td1 + td2` |
| Умножить на число | `td * 3` |
| Разделить на число | `td / 2` → `timedelta` |
| Разделить на timedelta | `td1 / td2` → `float` |
| Остаток от деления | `td1 % td2` |
| Модуль / отрицание | `abs(td)`, `-td` |
...
**⚠️ Ловушка с отрицательными интервалами:** Python нормализует `timedelta` так, что `days` может быть отрицательным, а `seconds`/`microseconds` — всегда неотрицательные. Из-за этого печатное представление отрицательного интервала выглядит неожиданно:

```python
timedelta(minutes=-40)
# -1 day, 23:20:00   — а НЕ "-0:40:00", как можно было бы подумать!
```

Логика: -40 минут = -1 день + 23 часа 20 минут (день "занят в долг", а остаток — положительный). Не баг, а способ внутреннего хранения — не пугайся, если увидишь такое в выводе.
...
**У `timedelta` нет атрибутов `.hours`/`.minutes`** — только `.days`/`.seconds`/`.microseconds`. Чтобы достать часы и минуты, их нужно вычислить из `.seconds` вручную:

```python
def hours_minutes(td):
    return td.seconds // 3600, (td.seconds // 60) % 60

td = timedelta(days=7, hours=8, minutes=10, seconds=125)
hours, minutes = hours_minutes(td)   # 8, 12
```
...
**`==`/`!=` ведут себя иначе, чем `<`/`>`/`<=`/`>=` при сравнении с посторонним типом.** Равенство/неравенство просто вернёт `False`/`True`, без ошибки — а вот операторы упорядочивания выбросят `TypeError`, потому что "больше/меньше" для несравнимых типов не определено:

```python
from datetime import timedelta

td = timedelta(seconds=57)

td != 57      # True — просто разные типы, ошибки нет
td == '5'     # False — тоже просто False, без ошибки

td > 5        # ❌ TypeError: '>' not supported between instances of 'datetime.timedelta' and 'int'
```

Из-за этой асимметрии легко пропустить логическую ошибку в коде: `if td != some_number:` тихо сработает и всегда будет `True`, хотя намерение было явно другим.
