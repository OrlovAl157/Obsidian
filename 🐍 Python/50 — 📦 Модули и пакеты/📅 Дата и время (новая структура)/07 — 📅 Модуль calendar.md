...
**Асимметрия в русской локали:** `day_name` выводится с маленькой буквы (`понедельник`), а вот `day_abbr` — сразу с большой (`Пн`, `Вт`). Так и задумано, не баг — просто у полного и сокращённого названия разные конвенции написания в русском языке.
