...
> [!tip] Запомни
> `name` — всегда строка с именем атрибута (`'MONDAY'`)
> `value` — то что присвоили (`1`, `'small'` и т.д.)
...
> [!tip] Вызов через () — не создание экземпляра
> `Weekday(1)` не создаёт новый объект — а **ищет существующий** элемент по значению. Создание экземпляров класса перечисления напрямую невозможно.
...
> [!tip] Рекомендация
> Используй `is` и `is not` — они быстрее, так как каждый элемент является **синглтоном** (существует в единственном экземпляре).
...
> [!tip] Помни
> Элементы перечисления — это **экземпляры класса** перечисления. Поэтому `self` в методах — это сам элемент (`Weekday.SUNDAY`).
...
> [!warning] Важно
> Результат арифметики — всегда `int`, даже если такое значение есть в перечислении.
> Чтобы получить элемент — конвертируй явно:
> ```python
...
| | `Enum` | `dict` | `namedtuple` |
|---|---|---|---|
| Защита от изменений | ✅ | ❌ | ✅ |
| Итерирование | ✅ | ✅ | ✅ |
| Защита от опечаток (IDE) | ✅ | ❌ | ✅ |
| Синглтон | ✅ | ❌ | ❌ |
| Сравнение через `is` | ✅ | ❌ | ❌ |
| Арифметика | только IntEnum | ❌ | ❌ |
